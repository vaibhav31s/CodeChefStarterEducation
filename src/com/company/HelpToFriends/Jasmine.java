package com.company.HelpToFriends;
...
//
//            out.println(getAnswer(new int[][]{{1,4},{1,5},{5,6},{6,10},{7,9}}));
//
//            out.println(getAnswer(new int[][]{{2,8},{3,9},{5,11},{3,4},{11,15}}));
//
//
//            out.println(getAnswer(new int[][]{{2,8},{3,7},{4,6},{1,9},{2,5}}));
            ...
            out.println(minOperations(list, q));

        }
    }


    public  static int getAnswer(List<List<Integer>> meetingTiming) {
        List<Integer> start = new ArrayList<>();
        List<Integer> end    = new ArrayList<>();

        for (List<Integer>  meeting : meetingTiming) {
            start.add(meeting.get(0));
            end.add(meeting.get(1));
        }
    ...
    public static List<Long> minOperations(List<Integer> prices , List<Integer> query  ) {
        int n = prices.size();
        ...
        for(int val : query) {
            int i = Collections.binarySearch(prices, val);
            if(i < 0) i = -(i + 1);
            ans.add((long) val * (2L * i - n) + prefix[n] - 2 * prefix[i]);
        }
        return ans;
    }
