    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0) {
            ...
            for(int i = max; i < max + 10; i++) {
                out.println(Arrays.toString(arr)  + " " +  (i)  + " " + getSum(arr, atMost - 1, i));
            }

//            TreeMap<Integer, Integer> map = new TreeMap<>();
//            for(int x :  arr) push(map, x, 1);
//            long answer  = 0;
//            int a = map.firstKey();
//            while(a != max) {
//                int count = map.get(a);
...
//                  dif = count - atMost;
//                  pull(map, a, dif);
//                  answer += (long) (max - a) * atMost;
//                } else  {
//                    int currentlyHaving = count;
//                    int currentlyNeeded = atMost - count;
//                    a = map.firstKey();
//                    int curCount= map.get(a);
//                    if(curCount > map.get())
//
//                    while(a != max) {
//
...
//        }
    ...
    public static long getSum(int[] arr, int atMost, int max) {
        long answer =0;
        int curMin = arr[0];
        int win = atMost;
        for(int i = 0; i < arr.length; i++) {
            if(win == 0) {
                answer += (long) (max - curMin);
                curMin = arr[i];
                win = atMost - 1;
            }
            if(arr[i] == max) break;
