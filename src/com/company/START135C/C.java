...
//            int[] arr = readIntArray(sc, n);
              ...
              int x = sc.nextInt(), b = sc.nextInt(),k = sc.nextInt();
              HashMap<Long, Long> dp = new HashMap<>();
//              out.println(x + " " + b + " " + k + " " + getAns(x, x, b, k, dp));
      out.println(getAns(x,x, b, k, dp));
    }

  }
  static long getAns(long a, long x, long b, long k, HashMap<Long, Long> dp) {

    if (x > b) return 0;
    if (x == b) return 0;
    if (dp.containsKey(x)) return dp.get(x);
    if ((b - x)  % k == 0) {
      long count =  1 + getAns(a, x * k, b, k, dp);
      dp.put(x, count);
      return count;
    }

   long count =  1 +  Math.min(getAns(a, x + 1, b, k, dp )   , getAns(a, x * k ,b, k, dp));
   dp.put(x, count);
   return count;
  }
