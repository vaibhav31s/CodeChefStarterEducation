...
class C {
  ...
  public static void main(String[] args) {
    ...
    while (t-- > 0) {
            int n = sc.nextInt();
            int m = sc.nextInt();
            int[] f = readIntArray(sc, n, m);
            int[] s = readIntArray(sc, n, m);

            HashMap<Integer, Integer> map = new HashMap<>();
            for (int  x : f) push(map, x, 1);

            long answer = 0;
            for (int x : s) {
              for (int i = 0; i < 100; i++)
                if (map.containsKey(m * i - x)) answer += map.get(m * i - x);
            }
...
//            int[] arr = readIntArray(sc, n);
  ...
  private static int[] readIntArray(FastScanner sc, int n, int m) {
    int[] arr = new int[n];
    for (int i = 0; i < n; i++) arr[i] = sc.nextInt() % m;
    return arr;
  }
