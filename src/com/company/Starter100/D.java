...
class D {
    ...
    public static void main(String[] args) {
        ...
        // int[] primes = sieveEratosthenes(100001);
        sieveOfEratosthenes(100001);
        int t = sc.nextInt();
        outer:
        while (t-- > 0) {
            N =sc.nextInt(); // Change N to the desired value

            used = new boolean[N + 1];
            permutation = new int[N];
////            int[] arr = readarr(sc, n);
//            out.println(Arrays.toString(Arrays.copyOfRange(primes, 0, n)));
    ...
    private static boolean validPer(int index) {
        if (index == N) {
            return true;
        }
        for (int i = 1; i <= N; i++) {
            if (!used[i] && isPrime[Math.abs(i - (index + 1))]) {
                permutation[index] = i;
                used[i] = true;
                if (validPer(index + 1)) {
                    return true;
