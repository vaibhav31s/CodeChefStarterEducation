package com.company.Starter104;
...
public class E {
    ...
    public static int ans(int[] mountain) {
        int n = mountain.length;

        if (n < 3) {
            return Arrays.stream(mountain).sum(); // Not enough elements to have valleys and peaks
        }
        ...
        for (int i = 0; i < n; i++) {
            while (!stack.isEmpty() && mountain[i] < mountain[stack.peek()]) {
                int valleyStart = stack.pop();
                int minElevation = mountain[valleyStart];
                while (!stack.isEmpty() && mountain[stack.peek()] == minElevation) {
                    stack.pop(); // Remove duplicate valley elements
                }

                // Flatten the valley
                for (int j = valleyStart; j <= i; j++) {
                    mountain[j] = minElevation;
                }
            }
            stack.push(i);
        }

        for (int elevation : mountain) {
            totalSum += elevation;
        }
