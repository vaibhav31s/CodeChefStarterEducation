...
public class C {
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            int land = 0;int mina = Integer.MAX_VALUE;

            for (int x : map.keySet()) {
                int min  =  go1(map.get(x), new HashSet<>(), n);
...
//                out.println(x + " " + min  );
            ...
            out.println(land + " " + mina);
//            out.println("------");
        }
    }

    private static int go1(ArrayList<Integer> integers, HashSet<Object> objects, int n) {
            int max = 0;
            int last = integers.get(0);
            if(last != 0){
                max = last;
            }
            int end = integers.get(integers.size()-1);
            if(end != n - 1) {
                max = Math.max(max, n - 1 - end);
            }
            for(int i = 1; i < integers.size(); i++){
                int first = integers.get(i - 1);
                int second =integers.get(i);
                if(second - first <= 1) continue;
                max = Math.max((second - first)/2, max);
            }
