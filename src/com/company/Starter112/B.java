...
class B {
    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0){
            int n = sc.nextInt();
            int[] typse = readIntArray(sc, n);
            int[] values = readIntArray(sc, n);
            HashMap<Integer, Integer> map = new HashMap<>();
            for(int i = 0; i < n; i++) {
                int types = typse[i];
                int value = values[i];
                map.putIfAbsent(types, 0);
                map.put(types, Math.max(map.get(types), value));
            }
            long answer =0;
//            out.println(map);
            for(int value : map.values()) answer += value;
            out.println(answer);
        }
    }
