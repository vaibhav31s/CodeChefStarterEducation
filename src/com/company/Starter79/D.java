...
//            int[] arr = readarr(sc, n);
                int x = sc.nextInt();
                int y = sc.nextInt();
               boolean a =  remove(x, y,false);
               if(a) out.println("CHEFINA");
               else out.println("CHEF");
        }
    }

    private static boolean remove(int x, int y, boolean b) {
        if(x<= 0 || y <= 0){
            if(b) return true;
            return  false;
        }
        if(b){
           boolean a =  remove(x-1, y, !b);
            boolean bb = remove(x-2, y, !b);
            boolean c =  remove(x, y-1, !b);
            boolean d =   remove(x, y-2, !b);
            return a ^ bb ^ c ^ d;
        }else{
            boolean a =  remove(x-1, y, !b);
            boolean bb = remove(x-2, y, !b);
