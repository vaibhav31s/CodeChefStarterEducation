...
public class FF {
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            long answer = 0;
            long ans = 0;
            long total = seg.query(0, n);
            for(int i = 0; i < n; i++){
                int first = arr[i];
                int second = n ;
//                if(second == -1) continue ;
                    long sum = seg.query(i+1, second-1);
                    int len = n - (second- 1 -  i  );
                    if(len >= 0){
                        long expectedsum = ((long) len * (len+1))/2;
                        if(total - sum ==  expectedsum){
                            ans = Math.max(second - i - 1 , ans);
                        }
                    }
                second = map.getOrDefault(first+1, n);
                sum = seg.query(i+1, second-1);
                len = n - (second- 1 -  i  );
                if(len >= 0){
                    long expectedsum = ((long) len * (len+1))/2;
                    if(total - sum ==  expectedsum){
