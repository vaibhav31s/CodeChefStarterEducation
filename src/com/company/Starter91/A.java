...
public class A {
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            ArrayList<KeyValue<Integer,Integer>> pairs=  new ArrayList<>();

            for(int i=1; i<n;i++)brr[i]+=brr[i-1];
            print(brr);
            for(int i = 0; i < n; i++){
                pairs.add(new KeyValue<>(arr[i], brr[i]));
            }
            sort(brr);
//            print(brr);
            sort(arr);
