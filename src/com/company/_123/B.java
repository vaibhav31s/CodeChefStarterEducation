...
class B {
    ...
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            precalculatePermutations(arr);
            TreeMap<Long, List<Integer>> map = getPermutationSums(ans);
            long[] queries = readLongArray(sc, q);
            processQueries(map, queries);
        }
        out.close();
    }

    static void precalculatePermutations(int[] nums) {
        ans = new ArrayList<>();
        Arrays.sort(nums);
        ans.add(Arrays.stream(nums).boxed().collect(ArrayList::new, ArrayList::add, ArrayList::addAll));
    ...
    static TreeMap<Long, List<Integer>> getPermutationSums(List<List<Integer>> permutations) {
    ...
    static void processQueries(TreeMap<Long, List<Integer>> map, long[] queries) {
        for (long x : queries) {
            Map.Entry<Long, List<Integer>> entry = map.ceilingEntry(x);
            if (entry != null) {
                for (int xx : entry.getValue()) {
                    out.print(xx + " ");
                }
                out.println();
            } else {
