...
//            int[] arr = readIntArray(sc, n);
            int n = sc.nextInt();
            int k = sc.nextInt();
//            out.println(minCellsToColor(n, k));
            int max = 4 * n - 2;
    ...
    public static int minCellsToColor(int n, int k) {
        // If k <= 2n - 1, we need to color only along the diagonals
        if (k <= 4 * n - 2) {
            return 4 * n;
        } else {
            // Calculate the additional diagonals needed to be covered
            int additionalDiagonals = k - (4 * n - 2);
            // Calculate the number of cells needed to cover the additional diagonals
            int additionalCells = 2 * ((additionalDiagonals + 1) / 2);
            // Total cells needed to be colored
            return 2 * n + additionalCells;
        }
    }
