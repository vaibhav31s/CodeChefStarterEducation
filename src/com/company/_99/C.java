...
//            int[] arr = readIntArray(sc, n);
            ...
            boolean ans = goo(k, a, 1,0);

            out.println(ans);
        }
    }

    private static boolean  goo(int k, int a, int x, int loose) {


        if(a > 0) {
            if(a == loose || x< 0) return false;
            if(x > xx) return true;
            return goo(k, a, x + (x * (k - 1)), 0);
        } else {
            if(a == loose) return false;
            return goo(k, a, x, loose + 1);
        }
    }
