...
public class C {
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            for(long x :  arr){
                ...
                for(long i= 0; i < 66; i++){
                    long mask = 1L << i;
                    if(((x & mask) != 0) && ((b & mask) != 0)) {
                        count++;
                    }

                }
                if(count == Long.bitCount(b)){
                    list.add(x);
                    out.println(x);
                    for(long i= 0; i < 64; i++){
                        long mask = 1L << i;
    ...
    private static BigInteger[] readarr(FastScanner sc, int n) {
        BigInteger[] arr = new BigInteger[n];
        for (int i = 0; i < n; i++) arr[i] = new BigInteger(sc.next());
        return arr;
    }
