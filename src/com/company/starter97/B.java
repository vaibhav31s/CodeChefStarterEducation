...
public class B {
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            for(int i = 0; i < 3; i++)pq.add(arr[i]);
            while(pq.size() != 1) a+= pq.poll();

            pq.clear();



            for(int i = 3; i < 6; i++)pq.add(arr[i]);
            while(pq.size() != 1) b+= pq.poll();
            if(a == b) out.println("Tie");
            else if(a > b) out.println("Alice");
