...
//            int[] arr = readarr(sc, n);
            String s = sc.next();
            Stack<Character> smilyStore = new Stack<>();
            int count = 0;
            for(char ch  : s.toCharArray()) {
                if (smilyStore.isEmpty() && ch == ':') {
                    smilyStore.add(':');
                } else if (!smilyStore.isEmpty() && (smilyStore.peek() == ')' || smilyStore.peek() == ':')  && ch == ')') {
                    smilyStore.add(')');
                } else if (!smilyStore.isEmpty() && smilyStore.peek() == ')' && ch == ':') {
                    count++;
                    smilyStore.clear();
                    smilyStore.add(':');
                } else if (!smilyStore.isEmpty() && ch == '(') {
                    smilyStore.clear();
                }
